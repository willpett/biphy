/**
 * @file
 * This file contains the implementation of NexusTreeMonitor, used to save
 * information to file about the tree and additional variables at nodes, e.g. branch rates, 
 * in extended Newick format.
 *
 * @brief Implementation of NexusTreeMonitor
 *
...
NexusTreeMonitor::NexusTreeMonitor(const NexusTreeMonitor &m) : Monitor( m ), outStream(), tree( m.tree ), nodeVariables( m.nodeVariables ) {
...
/* Clone the object */
NexusTreeMonitor* NexusTreeMonitor::clone(void) const {
    
    return new NexusTreeMonitor(*this);
}


void NexusTreeMonitor::closeStream() {
...
void NexusTreeMonitor::monitor(long gen) {
...
void NexusTreeMonitor::openStream(void) {
...
void NexusTreeMonitor::printHeader() {
	outStream << "#NEXUS\nbegin trees;\n";
}


void NexusTreeMonitor::swapNode(DagNode *oldN, DagNode *newN) {
