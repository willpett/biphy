/**
 * @file
 * This file contains the declaration of DPPBetaSimplexMove, which performs the DPP move based on Neal (2000) Algorithm 8
 * this move changes the value assigned to each table
 *
 * @brief Declaration of DPPBetaSimplexMove
 *
 * (c) Copyright 2009- under GPL version 3
 * @date Last modified: $Date: 2012-05-11 14:54:35 +0200 (Fri, 11 May 2012) $
 * @author The RevBayes Development Core Team
 * @license GPL version 3
 * @version 1.0
 * @since 2012-07-20, version 1.0
 *
 * $Id: DPPBetaSimplexMove.h $
 */

#ifndef DPPBetaSimplexMove_H
#define DPPBetaSimplexMove_H

#include <ostream>

#include "Move.h"
#include "StochasticNode.h"

namespace RevBayesCore {

    class DPPBetaSimplexMove : public Move {
    
    public:
        DPPBetaSimplexMove(StochasticNode<std::vector<double> >* v, double a, double w);                                                                      //!< Internal constructor
    
        // Basic utility functions
        DPPBetaSimplexMove*							clone(void) const;                                                                  //!< Clone object
        void                                                    swapNode(DagNode *oldN, DagNode *newN);
        const std::string&                                      getMoveName(void) const;                                                            //!< Get the name of the move for summary printing
		bool													isGibbs(void) const;

    protected:
        void													performGibbsMove(void);                                                            //!< Perform move
        void													acceptMove(void);                                                                   //!< Accept the InferenceMoveSimple
        double													performMove(double& probRatio);                                                     //!< Perform the InferenceMoveSimple
        void													rejectMove(void);                                                                   //!< Reject the InferenceMoveSimple
    
    private:
        double													getCurrentLnProbabilityForMove(void);
		
		double													alpha;                                                                             //!< The scaling parameter of the move
        StochasticNode<std::vector<double> >*                   variable;
		double													safeExponentiation(double x);
 
    };
    
}



#endif

