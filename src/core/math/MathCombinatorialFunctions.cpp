/**
 * @file MathCombinatorialFunctions
 ...
 */
double Math::choose(double n, double k) {
    ...
    if (k < k_small_max)
        {
        int j;
        if(n-k < k && n >= 0 && Math::isInt(n)) k = n-k; /* <- Symmetry */
        if (k <	 0) return 0.0;
        if (k == 0) return 1.0;
        /* else: k >= 1 */
        r = n;
        for(j = 2; j <= k; j++)
            r *= (n-j+1)/j;
        return Math::isInt(n) ? floor(r + 0.5) : r;
    ...
    else if (Math::isInt(n))
        {
        if(n < k) return 0.;
        if(n - k < k_small_max) return choose(n, n-k); /* <- Symmetry */
        return floor(exp(Math::lfastchoose(n, k)) + 0.5);
        }
    /* else non-integer n >= 0 : */
    if (n < k-1)
        {
        int s_choose;
        r = Math::lfastchoose2(n, k, /* -> */ &s_choose);
        return s_choose * exp(r);
        }
    return exp(Math::lfastchoose(n, k));
}
...
double Math::lnChoose(double n, double k) {
    ...
    if (k < 2)
        {
        if (k <	 0) return Constants::Double::neginf;
        if (k == 0) return 0.;
        /* else: k == 1 */
        return log(fabs(n));
        }
    ...
    if (n < 0)
        ...
        /* else: n >= k+2 */
        return Math::lfastchoose(n, k);
        }
    /* else non-integer n >= 0 : */
    if (n < k-1)
        {
        int s;
        return Math::lfastchoose2(n, k, &s);
        }
    return Math::lfastchoose(n, k);
}


double Math::lfastchoose(double n, double k) {

    return -log(n + 1.) - Math::lnBeta(n - k + 1., k + 1.);
}

/* mathematically the same:
 less stable typically, but useful if n-k+1 < 0 : */
double Math::lfastchoose2(double n, double k, int *s_choose) {

    double r = lnGamma_sign(n - k + 1., s_choose);
    return Math::lnGamma(n + 1.) - Math::lnGamma(k + 1.) - r;
}
...
int Math::stirlingFirst(int n, int k) {
...
long long int Math::stirlingSecond(int n, int k) {
...
long long int Math::restrictedMultinomial(int n, int k) {
