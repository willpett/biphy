 ...
 */
double Math::binomialDeviance(double x, double np)
{
    double ej, s, s1, v;
    int j;
 ...
 */
int Math::chebyshev_init(double *dos, int nos, double eta)
{
    int i, ii;
    double err;
...
double Math::gamma_old(double x) {
	...
	if ( y <= 0.0 )
    {
		...
		if ( value != 0.0 )
        {
			if ( y1 != ( double ) ( ( int ) ( y1 * 0.5 ) ) * 2.0 )
				parity = true;
			fact = -Constants::PI / std::sin(Constants::PI * value);
			y = y + 1.0;
        }
		else
        {
 ...
 *      Statistics, 22.
 */
double Math::incompleteBeta(double a, double b, double x) {
...
double Math::incompleteGamma_old(double x, double alpha, double scale) {
 ...
 */
double Math::lnBeta(double a, double b)
{
    double corr, p, q;
    ...
    if (p >= 10) {
        ...
        return log(q) * -0.5 + Constants::LN_SQRT_2PI + corr
            + (p - 0.5) * log(p / (p + q)) + q * Math::log1p(-p / (p + q));
    }
    else if (q >= 10) {
...
double Math::lnGamma_old(double a) {
...
double Math::logGamma1p(double x) 
{
 ...
 */
double Math::logcf (double x, double i, double d,
 ...
 */
double Math::log_sum_exp(std::vector<double>& x, double max)
{
	double lse = 0.0;
 ...
 * Calculate the log of the sum of the exponentials of a list of numbers
 */
double Math::log_sum_exp(std::vector<double>& x)
{
	double max = x.front();
	for(std::vector<double>::iterator it = x.begin(); it != x.end(); it++)
		max = std::max(max,*it);
 ...
 * Calculate the log of the sum of the exponentials of a list of numbers
 */
float Math::log_sum_exp(std::vector<float>& x, float max)
{
    float lse = 0.0;
 ...
 * Calculate the log of the sum of the exponentials of a list of numbers
 */
float Math::log_sum_exp(std::vector<float>& x)
{
    float max = x.front();
    for(std::vector<float>::iterator it = x.begin(); it != x.end(); it++)
        max = std::max(max,*it);
