 ...
 * \throws 
 */
double Statistics::Helper::dppConcParamFromNumTables(double tables, double num){
 ...
 * \throws 
 */
double Statistics::Helper::dppExpectNumTableFromConcParam(double conp, double num){
    
    if ( conp <= 0.0 )
		throw Exception( "The concentration parameter must be larger than 0.0." );
	if ( num < 2.0)
		throw Exception( "The Dirichlet Process expects there to be more than 1 data element: number > 2." );
...
double Statistics::Helper::pointChi2(double prob, double v) {
    ...
    do {
        q = ch;
        p1 = 0.5 * ch;
        if ((t = Math::incompleteGamma(p1, xx, g)) < 0) {
            throw new Exception("Arguments out of range: t < 0");
        }
        p2 = p - t;
 ...
 */
int Statistics::Helper::poissonInver(double lambda, RandomNumberGenerator& rng) {
 ...
 * \throws Does not throw an error.
 */
int Statistics::Helper::poissonLow(double lambda, RandomNumberGenerator& rng) {
 ...
 */
int Statistics::Helper::poissonRatioUniforms(double lambda, RandomNumberGenerator& rng) {
	...
	if (p_L_last != lambda) {
		...
		mode = (int)lambda;
		p_g  = log(lambda);
		p_q = mode * p_g - Math::lnFactorial(mode);
	...
	while(1) {
		...
		if (x < 0 || x >= p_bound) 
			continue;
		k = (int)(x);
		lf = k * p_g - Math::lnFactorial(k) - p_q;
		if (lf >= u * (4.0 - u) - 3.0) 
			break;
		if (u * (u - lf) > 1.0) 
			continue;
 ...
 * \throws Does not throw an error.
 */
double Statistics::Helper::rndGamma(double a, RandomNumberGenerator& rng) {
...
//	return (r);
    ...
    if (!Math::isFinite(a) || a < 0.0 ) {
        throw Exception("Infinite parameters for rgamma.");
    }
 ...
 * \throws Does not throw an error.
 */
double Statistics::Helper::rndGamma1(double s, RandomNumberGenerator& rng) {
 ...
 * \throws Does not throw an error.
 */
double Statistics::Helper::rndGamma2(double s, RandomNumberGenerator& rng) {
